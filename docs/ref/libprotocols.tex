...
supporting reflexivity: if you create a wrapper, it should declare itself as
supporting the protocol (declaration API does this automatically for you).

Wrapper functions should only wrap to object types that support the protocol

symmetry not automatic; must define "unwrapper"

preferable to keep the "base" object around, separate from each "interface
pointer", to use for equality or identity comparison and to obtain alternate
interfaces.
...
contains \function{NO_ADAPTER_NEEDED} is equivalent to the same adpater path
