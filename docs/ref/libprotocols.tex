...
class ISimpleMapping(Interface):
...
The ``pure" style emphasizes the interface to the caller, and is not intended to
be subclassed for implementation.  Notice that the \code{self} parameter is not
included in its method definitions, and its method bodies are empty.  The ``ABC"
style, however, emphasizes implementation, as it is intended to be subclassed
...
Subclassing a subclass of \class{Interface} creates another \class{Interface},
that implies the first interface.  This means that any object that is considered
to provide the second interface, is considered to implicitly provide the base
interface.  For example:

\begin{verbatim%
}class ISimpleWriteMapping(ISimpleMapping):
...
The \code{ISimpleWriteMapping} interface implies the \code{ISimpleMapping}
interface.  Therefore, any object that provides \code{ISimpleWriteMapping} is
understood to also provide the \code{ISimpleMapping} interface.  The reverse,
however, is not true.
