...
\item[factoryMethod = \var{methodName}] \hfill \\ \versionadded{1.0}
...
\newpage
\subsection{Dynamic Protocols (NEW in 1.0)\label{protocols-generated}}
...
So, the \module{protocols} package includes a number of utility methods to make
these uses more convenient.
...
\subsubsection{Defining a protocol based on a URI or UUID}
...
\subsubsection{Defining a protocol as a subset of an existing type}
...
\subsubsection{Defining a protocol for a sequence}

XXX sequenceOf()

\subsubsection{Defining a protocol as a local variation of another protocol}
...
top-level \module{protocols} package.  They are arranged roughly in order of
their expected frequency of use, with more frequently-used items listed first.
...
The \member{subject} attribute of an \class{Adapter} instance is
the \var{ob} supplied to its constructor.  That is, it is the object being
adapted.
\end{memberdesc}

\begin{memberdesc}{protocol}
The \member{subject} attribute of an \class{Adapter} instance is
the \var{proto} supplied to its constructor.  That is, it is the protocol to
which the \member{subject} is being adapted.
\end{memberdesc}
\end{classdesc}
...
\begin{funcdesc}{protocolForURI}{uri} \versionadded{1.0}
Return a protocol object that represents the supplied URI or UUID string.  It
is guaranteed that you will receive the same protocol object if you call this
routine more than once with equal strings.  This behavior is preserved even
across pickling and unpickling of the returned protocol object.
...
section \ref{protocols-generated}.
\end{funcdesc}

\begin{funcdesc}{protocolForType}{baseType,
\optional{methods=(), implicit=False}} \versionadded{1.0}
Return a protocol object that represents the subset of \var{baseType} denoted
by \var{methods}.  It is guaranteed that you will receive the same protocol
object if you call this routine more than once with eqivalent paremeters.  This
behavior is preserved even across pickling and unpickling of the returned
...
found in section \ref{protocols-generated}.
...
\item The only adapters supported are \function{NO_ADAPTER_NEEDED} and
\function{DOES_NOT_SUPPORT}.  Calling \method{__adapt__()} on a Zope
\class{Interface} will thus only return the original object, or \constant{None}.
Note that this may change in future releases of Zope X3, as Jim Fulton has
expressed an intention to add an \method{__adapt__()} method using a global
adapter registry.   At that time, it may also become possible to register
other adapters via the \module{protocols} declaration API.

\item Zope interfaces do not conform to \module{protocols} package ``shortest
...
declarations (e.g. \code{classProvides}).  This may be a bug in the current
version.

\end{itemize}

The current implementation of support for Zope X3 interfaces is currently based
on the Zope X3 CVS version; it does not work with older releases such as the
``milestone 2'' release.
...
Importing this module enables experimental support for using Twisted 1.0.5
\class{Interface} objects with the \module{protocols} package, by registering
...
\begin{verbatim%
    ...
    def drop(self,thing):
        adapt(thing,IPrintable,factory=Unprintable).printOn(self)

class Trashcan:
    def drop(self,thing):
        adapt(thing,IDeletable,factory=Undeleteable).delete(self)

class User:
    advise(instancesProvide=[IDeletable,IPrintable])
    def drop(self,thing):
        adapt(thing,IMailable,factory=Unmailable).mailTo(self)

class File:
    advise(instancesProvide=[IDeletable,IMailable,IPrintable])
    def drop(self,thing):
        adapt(thing,IInsertable,factory=Uninsertable).insertInto(self)
\end{verbatim}

\class{Unprintable}, \class{Undeletable}, \class{Umailable}, and
\class{Uninsertable} are classes that implement the \method{printOn()},
\method{delete()}, \method{mailTo()}, and \method{insertInto()} methods by
...
technique of using a \var{factory} argument that provides an empty or
error-raising implementation of an interface, is an example of the \strong{null
object pattern}.)
...
\code{adapt(document,IHTMLWriter).writeHTML()}, and off you go.
